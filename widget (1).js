(function () {
  ...
  const STYLE = `
    ...
    .po-msg p { margin: 0 0 6px 0; }
    .po-msg p:last-child { margin-bottom: 0; }
    .po-msg ul { margin: 4px 0; padding-left: 18px; }
    .po-msg li { margin-bottom: 3px; }
  ...
  // Nettoie les résidus de Markdown (au cas où) et met en forme proprement :
  // gras -> texte simple, lignes commençant par "- " -> puces, retours à la ligne -> <br>
  function formatBotText(raw) {
    let safe = escapeHtml(raw);

    // Retire les ** et # résiduels sans les afficher
  ...
  function addMessage(role, text) {
    ...
    if (role === "user") {
      el.textContent = text; // jamais d'HTML pour le texte tapé par le client
    } else {
